...
Prior to running simulation, sparse traces should be generated. Basic arguments of the simulator:
 ...
 - `--trace_dir` - path to the directory that stores all of the traces (no need to change according to accelerator)
...
```bash
python main.py --accelerator focus --trace_dir ../algorithm/output --model llava_vid --dataset videomme
```
The simulation will produce results including execution time, energy breakdown, memory access, etc.


## Reproducing Results (Estimated time: 20 minutes)
We provide scripts to run simulation of Focus and baselines. Please set the `TRACE_DIR` to the `TRACE_META_DIR` used in trace generation in algorithm part, and set `OUTPUT_DIR` to your desired output directory.
...
Compute the on-chip area and power of Focus and baselines (Table 3 in paper):
```bash
python arch/accelerator.py --output_dir OUTPUT_DIR    # (Estimated time: 5 seconds)
```

Results saved to `accelerator_area_power_buffer.csv`

### Design Space Exploration Simulation

We conduct comprehensive design space exploration for Focus. We scan over different key hyper-parameters of Focus to evaluate its performance:
- GEMM m tile size
- Vector size
- Block size
...
```bash
python main.py --all_models_datasets --accelerator focus --quantization --trace_dir ../algorithm/output --output_dir results     # (Estimated time: 3 minutes)
```
...
python main.py \
    ...
    --output_dir results     # (Estimated time: 3 minutes)
```
...
```python
python utils/analysis.py     # (Estimated time: 1 minute)
```

Results saved to `results/figure_13.svg`
