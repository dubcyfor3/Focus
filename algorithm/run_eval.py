...
def parse_eval_args(arg_list=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to write the accuracy to the output file",
    )
    parser.add_argument(
        "--write_accuracy_table_name",
        type=str,
        default="accuracy.csv",
        help="The file name of the accuracy table to write to",
    )
    parser.add_argument(
        "--write_sparsity",
        action="store_true",
        default=False,
        help="Whether to write the sparsity to the csv file",
    )
    # Focus arguments
    parser.add_argument(
        "--focus",
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="The similarity threshold for the focus",
    )
    parser.add_argument(
        "--block_size",
        type=int,
        default=2,
        help="The match range for the focus",
    )
    parser.add_argument(
        "--frame_block_size",
        type=int,
        default=2,
        help="The frame match range for the focus",
    )
    parser.add_argument(
        "--alpha_list",
        type=str,
        default="",
        help="The alpha lists for the focus, split with ',' ",
    )
    parser.add_argument(
        "--selected_layers",
        type=str,
        default="",
        help="The selected layers for the focus, split with ',' ",
    )
    parser.add_argument(
        "--gemm_m_size",
        type=int,
        default=1024,
        help="The m dimension tile size for the focus",
    )
    parser.add_argument(
        "--vector_size",
        type=int,
        default=32,
        help="The vector size for the focus",
    )
    parser.add_argument(
        "--SEC_only",
        action="store_true",
        help="Whether to only consider semantic concentration",
    )
    # Focus export trace arguments
    parser.add_argument(
        "--export_focus_trace",
        action="store_true",
        help="Whether to export the focus",
    )
    parser.add_argument(
        "--trace_dir",
        type=str,
        default="./output",
        help="The directory to export the focus trace",
    )
    parser.add_argument(
        "--trace_name",
        type=str,
        default="focus_trace",
        help="The name of the focus trace",
    )
    parser.add_argument(
        "--trace_meta_dir",
        type=str,
        default="./output",
        help="The directory to export the trace meta data",
    )
    parser.add_argument(
        "--use_median",
        action="store_true",
        default=False,
        help="Whether to use the median sparsity index to export the focus trace",
    )
    # Frame fusion arguments
    parser.add_argument(
        "--frame_fusion",
        action="store_true",
        help="Whether to use frame fusion for the model",
    )
    parser.add_argument(
        "--ff_cost",
        type=float,
        default=0.3,
        help="The cost for the frame fusion",
    )
    # CMC arguments
    parser.add_argument(
        "--CMC",
        action="store_true",
        help="Whether to use CMC for the model",
    )
    parser.add_argument(
        "--CMC_threshold",
        type=float,
        default=-1.0,
        help="The threshold for CMC",
    )
    parser.add_argument(
        "--CMC_query_threshold",
        type=float,
    ...
    parser.add_argument(
        "--CMC_simple",
        action="store_true",
        help="Whether to use simple CMC for the model",
    )
    ...
    parser.add_argument(
        "--adaptiv",
        action="store_true",
        help="Whether to use adaptiv for the model",
    )
    parser.add_argument(
        "--adaptiv_threshold",
        type=float,
        default=-1.0,
        help="The threshold for adaptiv",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Quantization configuration as a JSON string. Example: '{\"load_in_8bit\": true}' or '{\"load_in_4bit\": true, \"bnb_4bit_compute_dtype\": \"float16\"}'",
    )
    parser.add_argument(
        "--load_in_8bit",
        action="store_true",
        help="Enable 8-bit quantization using BitsAndBytes",
    )
    parser.add_argument(
        "--load_in_4bit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use double quantization for 4-bit quantization",
    )
    parser.add_argument(
        "--bnb_4bit_quant_type",
        type=str,
...
def cli_evaluate_single(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if args.quantization_config:
        try:
            ...
            eval_logger.info(f"Using quantization config from JSON: {quantization_config}")
        except json.JSONDecodeError as e:
            eval_logger.error(f"Failed to parse quantization_config JSON: {e}")
            sys.exit(1)
    elif args.load_in_8bit or args.load_in_4bit:
        # Build quantization config from individual arguments
        quantization_config = {}
        if args.load_in_8bit:
            quantization_config["load_in_8bit"] = True
        if args.load_in_4bit:
            quantization_config["load_in_4bit"] = True
            quantization_config["bnb_4bit_compute_dtype"] = args.bnb_4bit_compute_dtype
            quantization_config["bnb_4bit_use_double_quant"] = args.bnb_4bit_use_double_quant
            quantization_config["bnb_4bit_quant_type"] = args.bnb_4bit_quant_type
        eval_logger.info(f"Using quantization config from arguments: {quantization_config}")
...
def get_score_from_results(args,results):
    if args.tasks == "videomme":
        return results['results']['videomme']['videomme_perception_score,none']['overall']
    elif args.tasks == "mlvu":
        return results['results']['mlvu']['mlvu_perception_score,none']
    elif args.tasks == "mvbench":
        return results['results']['mvbench']['mvbench_accuracy,none']
    elif args.tasks == "vqav2":
        return results['results']['vqav2_val']['exact_match,none'] * 100
    elif args.tasks == "mme":
        cognition_score = results['results']['mme']['mme_cognition_score,none'] if 'mme_cognition_score,none' in results['results']['mme'] else 0
        perception_score = results['results']['mme']['mme_perception_score,none'] if 'mme_perception_score,none' in results['results']['mme'] else 0
        return cognition_score + perception_score
    elif args.tasks == "mmbench":
        cn_dev_score = results['results']['mmbench_cn_dev']['gpt_eval_score,none']
        en_dev_score = results['results']['mmbench_en_dev']['gpt_eval_score,none']
        ru_dev_score = results['results']['mmbench_ru_dev']['gpt_eval_score,none']
        return (cn_dev_score + en_dev_score + ru_dev_score) / 3
    else:
        raise ValueError(f"Task {args.tasks} not supported for score extraction")

def save_score_to_main_csv(args, score):
    if not os.path.exists(args.trace_meta_dir):
        os.makedirs(args.trace_meta_dir)
    file_name = os.path.join(args.trace_meta_dir, args.write_accuracy_table_name)
    
    # Determine which column to write to based on args
    if hasattr(args, 'focus') and args.focus:
        if args.load_in_8bit:
            column_name = "INT8_Focus"
        else:
            column_name = "Focus"
    elif hasattr(args, 'CMC') and args.CMC:
        column_name = "CMC"
    elif hasattr(args, 'adaptiv') and args.adaptiv:
        column_name = "Adaptiv"
    elif hasattr(args, 'frame_fusion') and args.frame_fusion:
        column_name = "FF"
    else:
        if args.load_in_8bit:
            column_name = "INT8_Dense"
        else:
            column_name = "Dense"
    
    # Get model and dataset names in CSV format
    model_name = args.model
    dataset_name = args.tasks
    
    # Define expected columns
    expected_columns = ["Models", "Dataset", "Metric", "Dense", "FF", "Adaptiv", "CMC", "Focus", "INT8_Dense", "INT8_Focus"]
    ...
    if os.path.exists(file_name) and os.path.getsize(file_name) > 0:
        try:
            ...
            # If CSV is malformed or empty, create new DataFrame
            df = pd.DataFrame(columns=expected_columns)
    else:
        # Create empty DataFrame with expected columns
        df = pd.DataFrame(columns=expected_columns)
    
    # Find the row matching model and dataset
    # Handle NaN values and missing columns in comparison
    if len(df) > 0 and "Models" in df.columns and "Dataset" in df.columns:
        mask = (df["Models"].fillna("") == model_name) & (df["Dataset"].fillna("") == dataset_name)
    ...
    if matching_indices:
        ...
        if pd.isna(df.at[idx, "Metric"]):
            df.at[idx, "Metric"] = "Acc."
    else:
        # Create new row
        new_row = {
            "Models": model_name,
        ...
        new_row[column_name] = f"{score:.2f}"
...
def save_score_to_dse_csv(args, score):
    if not os.path.exists(args.trace_meta_dir):
        os.makedirs(args.trace_meta_dir)
    file_name = os.path.join(args.trace_meta_dir, args.write_accuracy_table_name)
    m_tile_size = args.gemm_m_size
    block_size = f"{args.frame_block_size}x{args.block_size}x{args.block_size}"
    vector_size = args.vector_size

    expected_columns = ["m_tile_size", "block_size", "vector_size", "accuracy"]
    if os.path.exists(file_name) and os.path.getsize(file_name) > 0:
        try:
            df = pd.read_csv(file_name)
            # Ensure all expected columns exist
    ...
    # Find matching row
    # Handle NaN values and missing columns in comparison
    if len(df) > 0 and "m_tile_size" in df.columns and "block_size" in df.columns and "vector_size" in df.columns:
        mask = (df["m_tile_size"].fillna("") == m_tile_size) & (df["block_size"].fillna("") == block_size) & (df["vector_size"].fillna("") == vector_size)
        matching_indices = df.index[mask].tolist()
    else:
        matching_indices = []
...
def print_results(args, results):
    ...
    if args.output_path:
        ...
        # get current time in the format 20230324_102841
        datetime_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Create a unique identifier using key arguments to avoid conflicts
        model_name = args.model.replace("/", "_").replace("-", "_")
        tasks_name = args.tasks.replace(",", "_").replace("/", "_")
    
        
        output_file = os.path.join(args.output_path, f"{datetime_str}_{model_name}_{tasks_name}_results.txt")

        if args.write_accuracy:
            score = get_score_from_results(args, results)
            if args.write_accuracy_table_name == "accuracy.csv":
                save_score_to_main_csv(args, score)
            elif args.write_accuracy_table_name == "dse_a_m_tile_accuracy.csv":
                save_score_to_dse_csv(args, score)
            elif args.write_accuracy_table_name == "dse_c_block_accuracy.csv":
                save_score_to_dse_csv(args, score)
            elif args.write_accuracy_table_name == "dse_b_vector_accuracy.csv":
                save_score_to_dse_csv(args, score)
            else:
                raise ValueError(f"Invalid write_accuracy_table_name: {args.write_accuracy_table_name}")
        
        with open(output_file, "w") as f:
            # write the args into the file
            f.write(f"{args}\n\n")
            f.write(evaluator.make_table(results))
            if "groups" in results:
                f.write(evaluator.make_table(results, "groups"))
