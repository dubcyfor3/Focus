...
def get_vllm_embedding_CMC(self, data):
    ...
    num_images, patch_num, _ = vision_hidden_states[0].shape
    image_bound = data["image_bound"][0]

    # image 64 \image slice 64 \slice slice 64 \slice \n ...
    image_token_start_index = image_bound[0, 0] - 1
    image_token_end_index = image_bound[-1, 1] + 1
    image_token_length = (image_token_end_index - image_token_start_index + 1).item()
    assert image_token_length % num_frames == 0
    original_length = vllm_embedding.shape[1]

    patch_height = 1
    patch_width = image_token_length // num_frames

    self.cmc.prepare(patch_type, patch_height, patch_width, image_token_start_index, image_token_end_index, image_token_length, original_length, vllm_embedding)
